    ...
    ensemble_metadata = list(yvar = c(yvv), 
                              xvar = paste0('FE_', c("ln_TFR", 
                                                     "edu")),
                              re_coef = NULL,
                              ar = c(0,1,2,3 ), 
                              ar_mod = c(1,3), 
    ...
    coef_priors = list('conv' = -1, 'FE_logit_pop15' = -1,
                        'scaled_c' = -1,
                        'FE_logit_pop65' = 1,
    ...
    ## Save them in root
    save(list = c('metadata_list',  'GDPs_full_grid', 'ensemble_metadata', 'coef_priors'), file = paste0(root_fold, '/summary_files/mean_array.Rdata'))
    ...
    # arrays = '1:2'
    qsub_mean = paste0('qsub -N mean_job_', comment, ' ',
                        proj,
                        slots,
                        ' -t ' , arrays,
                        ' /share/singularity-images/health_fin/forecasting/shells/health_fin_forecasting_shell_singularity.sh ',
                        mean_script,
    ...
    rmse_distro = chaos_draw_distro_DMC(data = data_models_passed, chaos_pc = 10, metadata = ensemble_metadata,
                                       iso_portion = 1, region_portion = 0, super_region_portion = 0, N_draws = N_draws)  
    ...
    qsub_draws = paste0('qsub -N draws_job_', comment, ' ',
                         proj,
                         slots,
                         ' -t ' , arrays,
                         ' -j y -o /share/temp/sgeoutput/sadatnfs/output ',
    ...
    chaos_qsubber = function(oos, chaos) { 
      if(chaos) { 
        ...
        paste0('qsub -N ', paste0('chaos_job_', comment, '_', oos ),
               proj,
               slots,
               ' -j y -o /share/temp/sgeoutput/sadatnfs/output ',
               shell_file,
    ...
    ########################################
      ...
      ## Diff the draws BEYOND the first year only!
        yvar_draws[, paste0('draw_', c(1:metadata_list$N_draws)):= lapply(paste0('draw_', c(1:metadata_list$N_draws)), 
                                                                              function(v) log(get(v)) )]
        
        yvar_draws[, paste0('diff_draw_', c(1:metadata_list$N_draws)):= lapply(paste0('draw_', c(1:metadata_list$N_draws)), 
                                                                                   function(v) get(v) - data.table::shift(get(v)) ), by = 'iso3']
        
        yvar_draws[year > metadata_list$start_year, paste0('draw_', c(1:metadata_list$N_draws)):= lapply( paste0('diff_draw_', c(1:metadata_list$N_draws)),
                                                                                          function(v) (get(v)))]
    ...
    ## Use population data to either divvy or multiply
      pop_data = fread(paste0('/share/resource_tracking/forecasting/', variable, '/input_data/RT_2018_GDP_use.csv'))[
        year %in% c(metadata_list$start_year: metadata_list$end_FC), .(iso3, year, total_pop  )]
    ...
    scenario_mean_lines = mean_scenario_model(correlated_output$stats, metadata_list, aroc_years = 25, transform = 'log')
    
    ### Propagate draws
    scenario_draws = scenario_uncertainty(mean_data = scenario_mean_lines, 
                                           draws_data = correlated_output$draws, 
                                           metadata_list, transform = 'log')
    
    ## Save out
    write_feather(scenario_draws[['draws']], paste0(root_fold, '/summary_files/scenario_draws.feather'))
    fwrite(scenario_draws[['stats']], paste0(root_fold, '/summary_files/scenario_stats.csv'))
    
    
    
    ########################################
    #### (8) Convert per caps to labour (need labour frax)
    ########################################
    
    if(yvv == 'ln_gdplab') {
      
      print("Convert per caps to labour")
      
      ## Get frax  
      pop_data2 = fread(paste0('/share/resource_tracking/forecasting/', variable, '/input_data/RT_2018_GDP_use.csv'))[year %in% c(metadata_list$start_year: metadata_list$end_FC), 
                                                                                        .(iso3, year, labour_frac)]
      
      ## We will simply divide the GDPpc draws with the labour frac
      perlab_draws = scenario_draws$draws
      
      ## Merge with draws and multiply
      perlab_draws = merge(perlab_draws, pop_data2, c('iso3', 'year'), all.x= T)
      
      perlab_draws[, paste0('draw_', c(1:metadata_list$N_draws)):= lapply(paste0('draw_', c(1:metadata_list$N_draws)), function(x)
                                                                          get(x) / labour_frac)]
      perlab_draws[, labour_frac:= NULL]
      
      ## Make stats
      perlab_stats = stat_maker(perlab_draws, melt = T, merge = F, idvar = c('iso3', 'scenario', 'year'))
      
      ## Save out
      write_feather(perlab_draws, paste0(root_fold, '/summary_files/perlab_scenario_draws.feather'))
      fwrite(perlab_stats, paste0(root_fold, '/summary_files/perlab_scenario_stats.csv'))
      
    }
    
    
    ########################################
    #### (10) Upload stats to DB
    ########################################
      ...
      upload_prepz = prep_for_db_uploading(root_fold = root_fold, 
