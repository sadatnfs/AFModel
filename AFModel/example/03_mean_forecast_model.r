    ...
    ################################################
        ...
        ## Load the TMB model
        dyn.load(dynlib("~/forecasting/AFModel/AFModel"))
        ...
        system.time(tryCatch(output_TMB <-  run_model(data_params = data_params, 
          ...
          error = function(e) {
            ...
            record_model_and_stop(model, variable = 'poverty', comment, date  , type = 'failed')
        }))
        ...
        ### Forecast
        mean_forecast <- make_forecast(mean_est_data_obj = mean_est_data, tmb_output_object = output_TMB, tmb_data_param = data_params, add_residual = F)
        ...
        ## Test for growth rate bounds
          ...
          sfa_model <- sfa_bounds(data = input_data, depvar =  model_info$yvar, transform = 'logit', panelvar = 'iso3', yearvar='year')
    ...
    ######################################################
          ...
          save(list = c("postfile_iso", "output_TMB",  "mean_forecast"),
          ...
          record_model_and_stop(model, variable = 'poverty', comment, date  , type = 'passed')
