    ...
    suppressMessages(pacman::p_load(TMB, data.table, ggplot2, Matrix, feather, parallel, compiler, argparse))
    ...
    ## This function will take in the full array grid and return a list with all the specs
    ...
    make_data_param <- function(input_data, 
      ...
      if(!is.null(xvar)) {
        ...
        x_input = reshape2::acast(data.table::melt(input_data[!is.na(get(yvar)) & year >= start_year & year <= end_fit, 
                                                   ...
                                                   c('iso3', 'year'), variable.name = 'xvar'), xvar ~ iso3 ~ year)
      ...
      if(!is.null(re_vars)) {
        ...
        re_coef_input = reshape2::acast(data.table::melt(input_data[!is.na(get(yvar)) & year >= start_year & year <= end_fit, 
                                                         ...
                                                         c('iso3', 'year'), variable.name = 're_vars'), re_vars ~ iso3 ~ year)
      } else {
        re_coef_input = NA
        re_coef = 0
      }
    ...
    prep_data_for_forecast <- function(tmb_obj,
      ...
      if (!is.null(specs$xvar) | length(specs$xvar)>0) {
        pred_data[, paste0('b_', dimnames(tmb_data_param$data$x_input)[[1]]):= lapply(c(1:length(tmb_params[rn=='b', V1])), 
      ...
      if(specs$z == T | specs$z_coef == 1 | specs$rho >= 1) {
        ...
        if(specs$z_coef == 1) {
          ...
          z_coef_out <- data.table(iso3 = rownames(tmb_data_param$data$Y_input), 
        ...
        if(specs$rho >= 1) {
          ...
          if(specs$ar_mod %in% c(2,3)) {
            ...
            ar_out <- data.table(iso3 = rownames(tmb_data_param$data$Y_input), 
    ...
    make_forecast <- function(mean_est_data_obj, tmb_output_object, tmb_data_param, add_residual = T, transform = NULL, ...) {
      ...
      if(add_residual) {
        ...
        ## Simulate errors
        rw_errors <- simulate_rw_forecast(tmb_output_object = tmb_output_object, tmb_data_params = tmb_data_param, 
    ...
    test_sig <- compiler::cmpfun(function(tmb_output_object, alpha = 0.1, intercept_test = T, rigorous = F) {
      ...
      if(!rigorous) {
        pval_DT <- pval_DT[param %in% c('b', 'a', 'logSigma', 'logggroup', 'loggrcoef', 'c', 'rho_global', 
